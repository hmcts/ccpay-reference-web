...
class RequestPromiseLoggingHandler {
  constructor (requestPromise, apiLogger) {
  ...
  get (target, key) {
    if (contains(httpCallMethods, key)) {
      ...
      return (...args) => {
        ...
        return originalMethod.apply(this.requestPromise, args)
      }
    } else {
      return target[key]
    }
...
/**
 * Request-promise provides a convenience method which accepts an URI string and builds the options
...
module.exports = RequestPromiseLoggingHandler
